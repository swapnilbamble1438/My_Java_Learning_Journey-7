...
public class CandidatesController {
	...
	public String addvoters(@RequestParam("candidate") String candidate, @RequestParam("email") String email,
	...
	//===============================================
	// View Candidates Details Page

		@GetMapping("/viewcandidates")
		public String viewcandidates(Model m,HttpSession session,HttpServletRequest request) {
			...
			if (session != null) 
			{
				...
				if (session.getAttribute("admin") != null) 
				{
					if (session.getAttribute("admin").equals("admin"))
					{
							...
							List<Candidates> cnd = cndserv.getAllCandidates();
							m.addAttribute("cnd", cnd);
		...
		// Edit Candidates Page
		@GetMapping("/editcandidate/{id}")
		public String editcandidate(@PathVariable int id, Model m,HttpSession session,HttpServletRequest request) {
			...
			if (session != null) 
			{
				...
				if (session.getAttribute("admin") != null) 
				{
					if (session.getAttribute("admin").equals("admin"))
					{
						...
						Candidates cnd = cndserv.getCandidatesById(id);
						m.addAttribute("cnd", cnd);
		...
		public String updatecandidate(@ModelAttribute Candidates cnd, HttpSession session) {
			cndserv.addCandidates(cnd);

			session.setAttribute("msg", "Candidate Data updated successfully...");

			return "redirect:/viewcandidates";
		}

		@GetMapping("/deletecandidate/{id}")
		public String deletecandidate(@PathVariable int id, HttpSession session) {
			cndserv.deleteCandidates(id);

			session.setAttribute("msg", "Candidate data deleted successfully...");
